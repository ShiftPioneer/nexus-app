...
export function BookshelfTab() {
  ...
  return <div className="space-y-6">
      ...
      {viewMode === "kanban" ? <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {Object.entries(booksByStatus).map(([status, books]) => <Card key={status} className="overflow-hidden">
              <CardContent className="p-4 bg-accent-dark">
                ...
                <div className="space-y-4">
                  {books.map(book => <BookCard key={book.id} book={book} onEdit={handleEdit} onDelete={handleDelete} />)}
          ...
          <TabsContent value="all" className="mt-4">
            <div className="space-y-4">
              {allBooks.map(book => <BookCard key={book.id} book={book} onEdit={handleEdit} onDelete={handleDelete} listView />)}
            </div>
          </TabsContent>
          
          <TabsContent value="reading" className="mt-4">
            <div className="space-y-4">
              {booksByStatus["Reading Now"].map(book => <BookCard key={book.id} book={book} onEdit={handleEdit} onDelete={handleDelete} listView />)}
            </div>
          </TabsContent>
          
          <TabsContent value="toread" className="mt-4">
            <div className="space-y-4">
              {booksByStatus["Not Yet Read"].map(book => <BookCard key={book.id} book={book} onEdit={handleEdit} onDelete={handleDelete} listView />)}
            </div>
          </TabsContent>
          
          <TabsContent value="finished" className="mt-4">
            <div className="space-y-4">
              {booksByStatus["Finished"].map(book => <BookCard key={book.id} book={book} onEdit={handleEdit} onDelete={handleDelete} listView />)}
            </div>
          </TabsContent>
        </Tabs>}
      
      <BookDialog open={dialogOpen} onOpenChange={setDialogOpen} onSave={handleAddBook} book={currentBook} />
    </div>;
}