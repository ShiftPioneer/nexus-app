...
const BookshelfTab = () => {
  ...
  const renderStars = (rating: number = 0) => {
    return <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map(star => <Star key={star} className={`h-3 w-3 ${star <= rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300 dark:text-gray-600"}`} />)}
      </div>;
  };
  const renderGridView = () => <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {filteredBooks.map(book => <Card key={book.id} className="cursor-pointer hover:shadow-lg transition-all duration-200 group h-fit" onClick={() => {
      ...
      setShowBookDialog(true);
          ...
          <CardContent className="pt-0 space-y-3">
            ...
            {book.relatedSkillsets && book.relatedSkillsets.length > 0 && <div className="flex gap-1 flex-wrap">
                {book.relatedSkillsets.slice(0, 2).map(skillset => <Badge key={skillset} variant="outline" className="text-xs">
                    {skillset}
                  </Badge>)}
                {book.relatedSkillsets.length > 2 && <Badge variant="outline" className="text-xs">
  ...
  const renderListView = () => <div className="space-y-2">
      {filteredBooks.map(book => <Card key={book.id} className="cursor-pointer hover:shadow-md transition-all" onClick={() => {
      setSelectedBook(book);
      setShowBookDialog(true);
    }}>
          <CardContent className="p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col items-end gap-1 flex-shrink-0">
                ...
                {book.description && <div className="text-xs text-muted-foreground max-w-xs text-right line-clamp-1">
                    {book.description}
                  </div>}
              </div>
            </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex items-center gap-4 flex-1 max-w-md">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search books..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-slate-950" />
          </div>
        </div>
