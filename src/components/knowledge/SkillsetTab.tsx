...
export function SkillsetTab() {
  ...
  const handleAddSkillset = (skillset: Skillset) => {
    if (currentSkillset) {
      ...
      setSkillsets([...skillsets, { ...skillset, id: Date.now().toString() }]);
    }
    setDialogOpen(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => { setCurrentSkillset(null); setDialogOpen(true); }} className="gap-1">
          <Plus size={18} />
          Add Skillset
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {skillsets.map(skillset => (
          <Card key={skillset.id} className={cn("overflow-hidden", getCardBorder(skillset.color))}>
            <CardContent className="p-6">
              <div className="flex flex-col">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <h3 className="font-bold text-xl">{skillset.name}</h3>
                    <p className="text-muted-foreground text-sm">{skillset.description}</p>
                  </div>
                ...
                <div className="mt-6 space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium">{skillset.mastery}%</span>
                  </div>
                  <Progress value={skillset.mastery} className="h-2" />
                </div>
                
                <div className="flex justify-between items-center mt-6 text-sm text-muted-foreground">
                  <span>Resources: {skillset.resourceCount}</span>
                  <span>Last practiced: {skillset.lastPracticed.toISOString().split('T')[0]}</span>
                </div>
