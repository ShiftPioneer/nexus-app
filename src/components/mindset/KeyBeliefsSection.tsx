  ...
  const handleSaveBelief = () => {
    ...
    if (currentBelief) {
      ...
      setBeliefs(beliefs.map(belief => 
        belief.id === currentBelief.id 
          ? { ...belief, belief: newBelief, explanation: newExplanation, lastEditedAt: new Date() } 
          : belief
      ));
      toast({
        ...
        <CardContent>
          {beliefs.length === 0 ? (
            ...
            <div className="space-y-4">
              {beliefs.map(belief => (
                <Card key={belief.id} className="bg-gradient-to-r from-green-500/10 to-green-500/5">
                  <CardContent className="p-6">
                    ...
                    {belief.explanation && (
                      <p className="text-sm text-muted-foreground">{belief.explanation}</p>
                    )}
                    <div className="flex items-center mt-4 text-xs text-muted-foreground">
                      <Calendar className="h-3 w-3 mr-1" />
                      <span>Last edited: {format(new Date(belief.lastEditedAt), 'MMM d, yyyy')}</span>
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div>
              ...
              <Input
                id="belief"
                placeholder="e.g., Every challenge contains an opportunity"
                value={newBelief}
            ...
            <div>
              ...
              <Textarea
                id="explanation"
                placeholder="Why do you hold this belief? How does it influence your actions?"
                rows={3}
