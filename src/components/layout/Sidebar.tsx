...
const Sidebar: React.FC<SidebarProps> = () => {
  ...
  const MobileSidebar = () => {
    return (
      <Sheet>
        ...
        <SheetContent side="left" className="p-0 bg-slate-950">
          <div className="h-full flex flex-col">
            <div className="p-4">
              <h1 className="text-xl font-bold text-[#FF5722] mb-4">NEXUS</h1>
            </div>
            ...
            <div className="border-t border-[#2A2F3C] p-4">
              <div className="flex items-center gap-3 mb-2" onClick={handleProfileClick}>
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="text-sm font-medium truncate text-white">{getUserName()}</span>
                  <span className="text-xs text-[#0FA0CE]">Pro Plan</span>
                </div>
              </div>
              <Button 
                variant="ghost" 
                className="text-slate-400 hover:text-white w-full justify-start mt-2"
      ...
      <div className="relative h-screen">
        <ShadcnSidebar 
          ...
          className={cn(
            ...
            "[&_[data-sidebar=content]]:scrollbar-none hidden lg:block"
          )} 
          style={{
            '--sidebar-width': '14rem',
          ...
          <SidebarFooter className="border-t border-[#2A2F3C] p-3 bg-slate-950">
            ...
            {!isCollapsed && (
              <Button 
                variant="ghost" 
                size="sm" 
                className="text-slate-400 hover:text-white w-full justify-start text-xs"
                onClick={handleSignOut}
              >
        ...
        {isCollapsed && (
          <motion.div 
            className="absolute top-1/2 -translate-y-1/2 left-[3.8rem] h-12 w-1 bg-[#2A2F3C] rounded-r cursor-pointer opacity-50 hover:opacity-100 transition-opacity hidden lg:block" 
            onClick={handleToggleCollapse} 
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.5 }}
  ...
  // Return the appropriate sidebar based on device
  return (
    <>
      <MobileSidebar />
      <DesktopSidebar />
