      ...
      {isCollapsed ? (
        ...
        >
          <img 
            src={LOGO_URL} 
            alt="NEXUS" 
            className="w-2 h-2 object-contain group-hover:drop-shadow-glow transition-all duration-200" 
          />
        </Button>
      ) : (
        <>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded flex items-center justify-center shadow-sm bg-primary/20 border border-slate-300">
              <img src={LOGO_URL} alt="NEXUS" className="w-2 h-2 object-contain" />
            </div>
            <span className="text-xs font-bold bg-gradient-to-r from-primary to-primary-600 bg-clip-text text-transparent">
              NEXUS
            </span>
          </div>
          <Button
