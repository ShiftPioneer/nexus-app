...
const SidebarHeader: React.FC<SidebarHeaderProps> = ({ isCollapsed, onToggle }) => {
  ...
  return (
    <div className={cn(
      "flex items-center bg-slate-950 flex-shrink-0 h-14 sm:h-16 overflow-hidden border-b border-slate-700/30",
      isCollapsed && !isMobile ? "justify-center px-2" : "gap-3 px-4"
    )}>
      {isCollapsed && !isMobile ? (
        <Button
          ...
          className="w-10 h-10 p-0 rounded-lg transition-all duration-200 hover:bg-primary/20 hover:scale-105 group text-slate-300 hover:text-white border-0"
        >
          <img 
            src={LOGO_URL} 
            alt="NEXUS" 
        ...
        <>
          ...
          <Button
            ...
            size="icon"
            onClick={onToggle}
            className="ml-auto h-8 w-8 sm:h-9 sm:w-9 hover:bg-slate-800 transition-all duration-200 text-slate-400 hover:text-slate-200 border-0 flex-shrink-0"
