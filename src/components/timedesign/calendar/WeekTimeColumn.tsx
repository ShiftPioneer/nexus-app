...
const WeekTimeColumn: React.FC<WeekTimeColumnProps> = ({
  ...
  return <div className="border-r border-slate-700">
      {hours.map(hour => <div key={hour} className="h-[72px] relative">
          <span className="absolute -top-3 w-full text-right pr-2 text-sm font-medium text-muted-foreground text-slate-300">
            {formatHour(hour)}
          </span>
          <div className="border-t border-slate-700 h-[36px]"></div>
          <div className="relative">
            <span className="absolute right-2 -top-3 text-xs text-muted-foreground text-slate-300">
              30
            </span>
