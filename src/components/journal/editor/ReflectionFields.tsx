...
const ReflectionFields: React.FC<ReflectionFieldsProps> = ({
  ...
  return <div className="space-y-3">
      {fields.map(field => <div key={field.key}>
          <Label htmlFor={field.key} className=" text-orange-600">{field.label}</Label>
          <Textarea id={field.key} placeholder={field.placeholder} value={reflections[field.key as keyof typeof reflections]} onChange={e => onReflectionChange(field.key, e.target.value)} rows={field.rows || 3} className="mt-1 resize-none bg-slate-900 border-slate-300" />
        </div>)}
    </div>;
};
