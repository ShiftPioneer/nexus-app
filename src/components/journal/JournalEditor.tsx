...
const JournalEditor: React.FC<JournalEditorProps> = ({
  ...
  return <div className="space-y-4">
      <div>
        <Label htmlFor="title" className=" text-orange-600">Title</Label>
        <Input id="title" name="title" placeholder="Entry title" value={entry.title} onChange={handleChange} className="mt-1" />
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="reflections" className="space-y-4">
          <ReflectionFields reflections={reflections} onReflectionChange={handleReflectionChange} />
        </TabsContent>
        
        <TabsContent value="notes">
          <div>
            <Label htmlFor="content">Journal Content</Label>
            <Textarea id="content" name="content" placeholder="Write your thoughts here..." value={notes} onChange={e => setNotes(e.target.value)} className="min-h-[300px] mt-1 resize-y" />
          </div>
        </TabsContent>
      </Tabs>
