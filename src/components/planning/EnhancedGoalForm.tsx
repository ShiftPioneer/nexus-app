...
const EnhancedGoalForm: React.FC<EnhancedGoalFormProps> = ({
  ...
  const handleToggleMilestone = (id: string) => {
    setMilestones(milestones.map(milestone => milestone.id === id ? {
      ...milestone,
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto bg-slate-950">
        <DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            field
                  ...
                  <FormControl>
                    <Input placeholder="Enter goal title" className="bg-slate-900/50 border-slate-700 text-slate-100 focus:border-orange-500" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />
            ...
            field
                  ...
                  <FormControl>
                    <Textarea placeholder="What do you want to achieve?" className="min-h-[80px] bg-slate-900/50 border-slate-700 text-slate-100 focus:border-orange-500" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              field
                    ...
                    <Select onValueChange={value => handleTimeframeChange(value)} value={field.value}>
                      <FormControl>
                        <SelectTrigger className="bg-slate-900/50 border-slate-700 text-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="progress" render={({
              field
            }) => <FormItem>
                    ...
                    <FormControl>
                      <Slider value={[field.value]} onValueChange={values => field.onChange(values[0])} max={100} step={1} className="w-full" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormItem className="flex flex-col">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant={"outline"} className={cn("pl-3 text-left font-normal bg-slate-900/50 border-slate-700 text-slate-100 hover:bg-slate-800", !startDate && "text-muted-foreground")}>
              ...
              <FormItem className="flex flex-col">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant={"outline"} className={cn("pl-3 text-left font-normal bg-slate-900/50 border-slate-700 text-slate-100 hover:bg-slate-800", !endDate && "text-muted-foreground")}>
            ...
            <div className="space-y-3">
              ...
              <div className="flex space-x-2">
                <div className="flex-1 flex space-x-2">
                  <Input value={milestoneInput} onChange={e => setMilestoneInput(e.target.value)} placeholder="Add milestone" className="flex-1 bg-slate-900/50 border-slate-700 text-slate-100 focus:border-orange-500" onKeyDown={e => {
                  if (e.key === "Enter") {
                    e.preventDefault();
                    handleAddMilestone();
                  }
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant={"outline"} className={cn("w-[130px] pl-3 text-left font-normal bg-slate-900/50 border-slate-700 text-slate-100 hover:bg-slate-800", !milestoneDueDate && "text-muted-foreground")}>
                        {milestoneDueDate ? format(milestoneDueDate, "PP") : <span>Due date</span>}
                        <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0 bg-slate-900 border-slate-700" align="start">
                      <Calendar mode="single" selected={milestoneDueDate} onSelect={date => date && setMilestoneDueDate(date)} initialFocus />
                    </PopoverContent>
                  </Popover>
                </div>
                <Button type="button" variant="outline" onClick={handleAddMilestone} className="bg-orange-500/10 border-orange-500/30 text-orange-400 hover:bg-orange-500/20">
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
              <div className="space-y-2 mt-2">
                {milestones.map(milestone => <div key={milestone.id} className="flex items-center justify-between bg-slate-800/50 p-3 rounded-lg border border-slate-700">
                    <div className="flex items-center gap-3">
                      <input type="checkbox" checked={milestone.completed} onChange={() => handleToggleMilestone(milestone.id)} className="rounded-sm w-4 h-4 text-orange-500 focus:ring-orange-500" />
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={() => handleRemoveMilestone(milestone.id)} className="h-6 w-6 p-0 text-red-400 hover:text-red-300 hover:bg-red-500/10">
            ...
            <DialogFooter>
              <Button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white shadow-lg shadow-orange-500/20">
