...
import { 
  ...
  CalendarDays, CheckSquare, Grid2X2, ListTodo, Filter 
} from "lucide-react";
...
const ActionsContent = () => {
  ...
  const getIncompleteTasks = (isToDoNot: boolean = false) => {
    return allTasks.filter(task => 
      (task.status === "today" || task.status === "todo" || task.status === "next-action") && 
      !isCompleted(task) && 
  ...
  const handleTaskSubmit = (taskData: any) => {
    ...
    if (editingTask) {
      updateTask(editingTask.id, { ...taskData, isToDoNot });
      toast({
        title: "Task Updated",
    ...
    >
      ...
      <Tabs defaultValue="todo" value={activeTab} onValueChange={(value) => setActiveTab(value as "todo" | "todo-not")} className="w-full">
        <TabsList className="mb-6">
          ...
          <TabsTrigger value="todo-not" className="flex-1">To Do Not</TabsTrigger>
        </TabsList>
  ...
  function renderTasksContent(isToDoNot: boolean) {
    return (viewMode === "eisenhower") ? (
      ...
      <>
        ...
        {viewMode === "kanban" && (
          <KanbanBoard
            columns={getKanbanColumns(isToDoNot)}
            onTaskClick={handleEditTask}
            onTaskMove={handleTaskMove}
            getPriorityColor={getPriorityColor}
            isToDoNot={isToDoNot}
        ...
        {viewMode === "list" && (
          <Tabs defaultValue="today">
            ...
            <TabsContent value="upcoming" className="mt-4">
              <TasksList 
                tasks={getIncompleteTasks(isToDoNot).filter(t => t.status !== "today")} 
                showActions={true}
                isToDoNot={isToDoNot}
              />
            </TabsContent>
            ...
            <TabsContent value="completed" className="mt-4">
              <TasksList 
                tasks={getCompletedTasks(isToDoNot)} 
                showActions={false}
                isToDoNot={isToDoNot}
              />
            </TabsContent>
